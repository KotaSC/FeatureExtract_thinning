...
void calculateFeature::calc(kvs::PolygonObject *ply)
{
  ...
  else if (m_type == InnerProductOfNormal)
  {
    calcInnerProductOfNormal(ply);
  }
  else if (m_type == DepthDisplacement)
  {
...
void calculateFeature::calcDepthDisplacement(kvs::PolygonObject *ply)
{
  ply->updateMinMaxCoords();
  kvs::ValueArray<kvs::Real32> coords = ply->coords();
  float *pdata = coords.data();
