...
FeaturePointExtractionThinning::FeaturePointExtractionThinning( void ) {
}

FeaturePointExtractionThinning::FeaturePointExtractionThinning( kvs::PolygonObject* ply,
                                                std::vector<float> &ft,
                                                double ft_ratio,
                                                double threshold )
{
...
void FeaturePointExtractionThinning::alpbaControl4Feature( kvs::PolygonObject* ply,
                                                   std::vector<float> &ft,
                                                   double ft_ratio,
                                                   double threshold )
{
  ...
  while( true ) {
    ...
    if( function >= r ){
      ...
      // SetColors.push_back( 255 );
      // SetColors.push_back( (1-ftNorm[index]) * 255 );
      // SetColors.push_back( (1-ftNorm[index]) * 255 );

      // SetColors.push_back( 255 );
      // SetColors.push_back( (1-function) * 255 );
      // SetColors.push_back( (1-function) * 255 );
